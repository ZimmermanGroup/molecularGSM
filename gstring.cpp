...
//cartesian rotation, incomplete
  ...
  grad1.init(infile0,natoms,anumbers,anames,icoords[1].coords,runNum,runend,ncpu,1);
  newic.grad_init(infile0,ncpu,runNum,runend-1,0);
#if !USE_MOLPRO
  for (int n=0;n<nnmax0;n++)
    icoords[n].grad_init(infile0,ncpu,runNum,runend+n,0); //level 3 is exact kNNR only, 0 is QM grad always
#else
  ...
  for (int n=0;n<nnmax0;n++)
  {
    ...
    icoords[n].grad_init(infile0,ncpu,runNum,runend+n,0);
...
void GString::scan_r(int weig)
{
  ...
  grad1.init(infile0,natoms,anumbers,anames,icoords[0].coords,runNum,runend,ncpu,3);
...
void GString::opt_tr()
{
  ...
  newic.grad_init(infile0,ncpu,runNum,runend-1,1);
  grad1.init(infile0,natoms,anumbers,anames,icoords[0].coords,runNum,runend,ncpu,3);
